...
(function() {
	...
	//var events = ("mousemove mouseover mouseout mousedown mouseup click vmousedown " + "dblclick focus blur submit change").split(" ");
	//var events = ("mousemove mouseover mouseout mousedown mouseup vmousedown " + "dblclick focus blur submit change").split(" ");
    ...
    jigsaw.Jigsaw = Class.extend({
        init: function(opts) {
            ...
            eventBus.on(jigsaw.Events.SHOW_EDGE, function() {
				console.log("canvas-puzzle.js, window.jigsaw SHOW_EDGE top");
				clearShowButtons();
				document.getElementById("SHOW_EDGE").classList.add("active");	
                self.ce.find("#middle").attr("hide", true);
                self.ce.find("#edge").attr("hide", false);
                self.ce.redraw()
            });
            eventBus.on(jigsaw.Events.SHOW_MIDDLE, function() {
				console.log("canvas-puzzle.js, window.jigsaw SHOW_MIDDLE top");				
				clearShowButtons();
				document.getElementById("SHOW_MIDDLE").classList.add("active");					
                self.ce.find("#middle").attr("hide", false);
                self.ce.find("#edge").attr("hide", true);
                self.ce.redraw()
            });
            eventBus.on(jigsaw.Events.SHOW_ALL, function() {
				console.log("canvas-puzzle.js, window.jigsaw SHOW_ALL top");
				clearShowButtons();				
				document.getElementById("SHOW_ALL").classList.add("active");					
                self.ce.find("*").attr("hide", false);
                self.ce.redraw()
            });

			function clearShowButtons () {
				document.getElementById("SHOW_EDGE").classList.remove("active");	
				document.getElementById("SHOW_MIDDLE").classList.remove("active");	
				document.getElementById("SHOW_ALL").classList.remove("active");									
			}
            function resizeView() {
                var cv1 = self.ce.cv;
...
//declare the maximum distance between the screen's limits and reference image's limit
//because the size of a piece differs for each size of the puzzle
				var limitRight = 0;	//how far should the pieces be displayed on the right side
				var limitLeft = 0;	//how far should the pieces be displayed on the right side

                var limitBottom = window.innerHeight - 20;	//to make sure no piece leaves the screen on Y axis
				F = T[i];
//just a check to see what size should the puzzle have, to set the maximum distance allowed				
			
				if(F.size > 160 && F.size < 250){ //2x2
					limitRight = window.innerWidth - 200;
					limitLeft = 180;
					
				}
				else if(F.size == 155){ //3x3
					limitRight = window.innerWidth - 300;
					limitLeft = 300;
					
				}
				else if(F.size > 94 && F.size < 142){ //Landscape 3x3
					limitRight = window.innerWidth - 300;
					limitLeft = 310;
				}
				else if(F.size == 117){ //4x4
					limitRight = window.innerWidth - 300;
					limitLeft = 300;
					
				}
				else if(F.size > 92 && F.size < 118){ // Random 4x4
					limitRight = window.innerWidth - 300;
					limitLeft = 300;
					
				}
				else if(F.size == 99){ //5x5
					limitRight = window.innerWidth - 320;
					limitLeft = 310;
					
				}
				else if(F.size == 67){ //Landscape 5x5
					limitRight = window.innerWidth - 320;
					limitLeft = 310;
					
				}
				else if(F.size == 87){ //6x6
					limitRight = window.innerWidth - 320;
					limitLeft = 280;
					
				}
				else if(F.size == 57){ //Landscape 6x6
					limitRight = window.innerWidth - 320;
					limitLeft = 280;
					
				}
				else if(F.size == 77){ //6x6
					limitRight = window.innerWidth - 350;
					limitLeft = 310;
				}
				else if(F.size > 50 && F.size < 80){ //Landscape 7x7
					limitRight = window.innerWidth - 280;
					limitLeft = 280;
					
				}		
				
					if(counter<(l/2)){
						F.tx = Math.floor((Math.random()*limitLeft) + 20);
						F.ty = Util.randint(h);
						if(F.ty <= 20){
							F.ty += Math.floor((Math.random()*100)+60);
						}
						if(F.ty >= limitBottom){
							F.ty -= Math.floor((Math.random()*50) + 20);
						}
						//if((F.ty + F.size) >= limitBottom){
						//	F.ty -= Math.floor((Math.random()*50) + 20);
						//}
					}
					if(counter>=l/2){
						
						F.tx = Util.randint(limitRight);
							if(F.tx <= (limitRight)){
								F.tx = limitRight + Util.randint(150) + 20;
							}
						F.ty = Util.randint(h);
						if(F.ty <= 20){
							F.ty += Math.floor((Math.random()*100)+60);
...
//***************
							borderSize: this.opts.borderSize,
